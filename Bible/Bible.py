...
def view_highlights():
	...
	can1.create_text(10,h-25,text=book_,font=("FreeMono",13),fill="#38fca5",anchor="w")
	...
	if len(ar)==0:
		...
		can2.create_text(int(can2["width"])/2,int(can2["height"])/2,
			text="No Record",font=("FreeMono",20),fill="#38fca5")
	...
	for i in ar:
		...
		can2.create_text(5,_y,text=txt,anchor="nw",font=("FreeMono",13),fill="#38fca5")
...
def main():
	...
	can1.create_text(int(can2["width"])/2,h-25,text=str(sel_chapter)+"/"+
		str(len(book[sel_book])),font=("FreeMono",13),fill="#38fca5")
	...
	for i in ar:
		...
		for t in art:
			...
			try:
				...
				can2.create_rectangle(3,_y-9,3+get_text_length(can2, t, "FreeMono", 13)+3,_y+9,fill="#38fca5",outline="#38fca5")
			except:
				pass

			can2.create_text(5,_y,text=t,anchor="w",font=("FreeMono",13),fill=col)
...
def sel_testament_():
	...
	for i in ar_testament:
		...
		can1.create_rectangle(int(can1["width"])/2-i[1]+15,i[2], int(can1["width"])/2+i[1]-15,i[2]+30-1,fill="#38fca5",outline="#38fca5")
...
def draw_round_rect(c,x1,y1,x2,y2,r,col):
...
def sel_book_():
	...
	#create_polygon(*[0,0, w,0, w,49, 0,49, 0,0], fill="#38fca5", alpha=0.1,can=can1)

	can1.create_text(w/2,20,text=sel_testament,font=("FreeMono",20),fill="#ffffff")

	can1.create_image(w-30-5,5,image=back,anchor="nw")

	#create_polygon(*[0,h-49, w,h-49, w,h, 0,h, 0,h-49], fill="#38fca5", alpha=0.1,can=can1)
	#can1.create_line(0,49,w,49,fill="#ffffff")
	...
	for i in books3:
		...
		can2.create_rectangle(int(can2["width"])/2-i[1]+15,i[2], int(can2["width"])/2+i[1]-15,i[2]+30-1,fill="#38fca5",outline="#38fca5")
...
def sel_chapter_():
	...
	#create_polygon(*[0,0, w,0, w,49, 0,49, 0,0], fill="#38fca5", alpha=0.3,can=can1)
	...
	s=90
	#create_polygon(*[0,h-49, w,h-49, w,h, 0,h, 0,h-49], fill="#38fca5", alpha=0.3,can=can1)
	...
	can1.create_rectangle(w/2-s+15,h-50+(50-30)/2,w/2+s-15,h-50+(50-30)/2+30-1,fill="#38fca5",outline="#38fca5")
	...
	for y_ in range(int(n)+n2):
		...
		for x in range(4):
			...
			draw_round_rect(can2,xx,y,xx+sz,y+30,15,"#ffffff")


			can2.create_text(xx+sz/2,y+15,text=str(c),font=("FreeMono",13),fill="#ffffff")
...
bg2=0
def load_im():
	global circle1,circle2,circle3
	global back
	global previous,next_
	global bg,bg2
